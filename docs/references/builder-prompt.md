...
PR description includes a full traceability table for the target domain, every run, every mode -- not just what changed this run. One row per spec requirement (every bullet in the domain's spec), plus, whenever the domain has a frontend (already existed, or built this run), three fixed frontend rows: initial list render, create-success flow, create-validation-error flow. Each row: requirement | Modified or Not modified | the test that backs it. "Modified" means a genuinely new test added in this diff; "Not modified" means an existing test, from an earlier PR, that still covers it unchanged -- name it anyway, don't omit the row just because this run didn't touch it. A requirement missing from the table entirely, or a row with no test name, means not done. This is what makes the table meaningful for frontend-only/frontend-update runs too: most or all backend rows there are legitimately "Not modified" (the backend was built and tested in an earlier PR) -- that's a complete, accurate table, not an empty one. check_builder_scope.py enforces a lower bound on new test count for the modes where new backend tests are expected (not frontend-only/frontend-update, where the component test is that mode's equivalent gate) -- but the table itself, "Not modified" rows included, must be genuinely accurate, not padded or omitted to pass.

**Test column format, exact, mechanically matched by `check_merge_gate.py`'s `TABLE_ROW_PATTERN` -- don't decorate it.** This paragraph is that pattern's format contract -- the two must be kept in sync by hand, nothing cross-checks them automatically, so if you're ever changing this paragraph, go update `TABLE_ROW_PATTERN` in the same PR (and vice versa): for a backend row, the bare test function name alone, e.g. `` `test_create_tag` `` -- no filename prefix (the file is always `tests/<domain>/test_service.py`, redundant to restate). For a frontend row, the bare test title alone, exactly as it appears inside the `it(...)`/`test(...)` call in `<Domain>.test.jsx`, e.g. `` `renders the list from tagsApi.list()` `` -- no filename prefix, no em dash, no separate quoting layer on top of the backticks. If one requirement is genuinely covered by more than one test function (e.g. a bullet with two distinct sub-cases), list them comma-separated in the same cell, e.g. `` `test_search_matches_case_insensitive_substring, test_search_no_match_returns_empty` `` -- never a `/`, never prose in between. The checker matches each name as a literal substring; any extra formatting around it is read as part of the name and will fail the match even when the underlying test is completely correct.
