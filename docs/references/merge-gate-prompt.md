...
3. Judge, narrowly, whether that specific test genuinely exercises that specific requirement -- not "does it look reasonable," but "does this assertion actually prove this claim." Three outcomes, not two (2026-08-26, see merge-gate-review-prompt.md): **fails** (asserts something unrelated, or a copy-paste of another domain's test with the names swapped but the behavior unchecked), **uncertain** (on-topic but proves a weaker claim than the requirement states, covers only the happy path when the requirement implies an edge case too, or is genuinely ambiguous against the requirement's exact wording), or passes. Both fails and uncertain block the merge identically -- never resolve genuine doubt by defaulting to pass.
Any Modified row that fails or is uncertain -> not eligible. Comment on the PR naming which row, its verdict (fails vs. needs human judgment), and why -- the verdict matters to whoever reads the comment even though the merge outcome is the same either way. Do not merge.
...
- Never merge a PR outside the eligible diff-shape boundary (`frontend_only`/`frontend_update`/`existing_domain`/`docs_only`, never `new_domain`), and never merge a PR that touches any critical path, even if it looks obviously fine by eye.
...
- ~~Widening eligibility to `docs_only` diffs~~ -- done (2026-08-26), after PR #115 (a genuine doc-only Maintenance PR: spec `Verified:` bumps, `findings-log.md`, `milestones.md`) turned up with no eligible shape at all. No code or tests in scope for that shape, so the traceability table and semantic review are both skipped -- mechanical-clean (critical-path-clear + CI green) is the whole bar.
