...
- List open PRs on `agentic-build/*` branches: `gh pr list --state open --json number,headRefName,title` filtered to `headRefName` starting with `agentic-build/`.
...
- Run `python3 scripts/check_merge_gate.py --pr <n> --mechanical-only` for each candidate PR. This checks: CI is green (reads `statusCheckRollup`, never re-runs ruff/pytest/etc. yourself -- CI already did that fresh), the diff shape matches `frontend_only`, and the PR body's traceability table has one row per spec bullet with each named test function genuinely present in the diff. Any nonzero exit means not eligible -- read its printed findings, do not override them, do not merge, move to the next PR.

SEMANTIC TRACEABILITY CHECK (the one thing the script can't do)
For each row in the PR body's requirement-to-test table that passed the script's structural check (function exists in the diff):
1. Quote the literal spec bullet text (read the target spec file at the PR's head commit: `git fetch origin pull/<n>/head` then `git show FETCH_HEAD:docs/product-specs/<domain>.md` -- never `gh pr checkout`).
2. Read the actual body of the named test function from `gh pr diff <n>`.
3. Judge, narrowly, whether that specific test genuinely exercises that specific bullet -- not "does it look reasonable," but "does this assertion actually prove this claim." A test that exists but asserts something unrelated, or a test that's a copy-paste of another domain's test with the names swapped but the actual behavior unchecked, fails this even though the mechanical check already confirmed the function exists.
Any row that fails this judgment -> not eligible. Comment on the PR naming which row and why, do not merge.

ACTIONS
- All rows pass (mechanical check clean + your own semantic check clean) -> `gh pr merge <n> --squash --delete-branch`. Confirm the merge actually succeeded (check the command's own exit code and `gh pr view <n> --json state` afterward) before considering it done -- a failed merge call is not a merge, report it, don't retry silently.
