...
| 2026-08-19 | `stale-doc-prose` | `docs/product-specs/tags.md` | agentic Routine (step 2 spec review) | spec said "list all tags, in creation order, since that's more useful for recent-first display" -- self-contradictory wording; `repo.py`'s `list_by_created` orders `created_at DESC` (newest first), and `list_tags_recent`'s naming plus `test_list_tags_recent_first` all confirm newest-first is the intended and actual behavior -- code was right, spec wording was stale/misleading | doc — corrected the Behavior bullet to "most-recently-created first", bumped `Verified:` to 2026-08-19; no rule (3rd occurrence of `stale-doc-prose`, but prose *correctness* still isn't mechanically checkable -- same reasoning as the 2nd-occurrence entry above; a golden rule can't judge whether English text like "in creation order" matches DESC-vs-ASC intent, only a semantic spec review can) |
