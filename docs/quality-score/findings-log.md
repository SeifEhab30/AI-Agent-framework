...
| 2026-08-25 | `stale-doc-prose` | `docs/product-specs/tags.md` | agentic Routine (step 2 spec review) | the "List all tags" bullet said "in creation order, since that's more useful for recent-first display" -- "creation order" and "recent-first" describe opposite orderings, and the actual implementation (`TagRepo.list_by_created()`, `ORDER BY created_at DESC`, confirmed by `test_list_tags_recent_first`) has always been newest-first; code was right, the bullet's wording was self-contradictory and stale | doc — reworded the bullet to "newest first (reverse creation order)", bumped `Verified:` to 2026-08-25; no rule (5th occurrence of `stale-doc-prose`, same reasoning as above -- doc-gardener's Verified-vs-source-commit-date check is already the standing recurring guard, prose correctness itself isn't mechanically checkable) |
| 2026-08-26 | `stale-doc-prose` | `docs/references/routine-prompt.md` | doc-gardener | "Next scheduled fire: 2026-08-24 (Tuesday 05:00 UTC)" was left over from the pre-retiming prompt version -- the cron was moved to `30 5 * * 2` on 2026-08-24 (same edit that wrote this line), so the line described a schedule that no longer existed the moment it landed, and by this run the date it named had already passed | doc — replaced the dated one-off line with the actual cron expression plus a note that the Dispatcher Routine also fires this Routine ad hoc between scheduled runs, bumped `Verified:` to 2026-08-26; no rule (6th occurrence of `stale-doc-prose`, same reasoning as above -- doc-gardener's Verified-vs-source-commit-date check is already the standing recurring guard, prose correctness itself isn't mechanically checkable) |
| 2026-08-26 | `stale-doc-prose` | `docs/references/merge-gate-review-prompt.md` | doc-gardener | file had no `Verified:` line at all since it was added for the merge-gate pilot (2026-08-25) -- every other doc in `docs/` carries one per `conventions.md`; content was checked against its two referenced files (`scripts/check_merge_gate.py`, `scripts/run-agent.sh`) and found accurate | doc — added `Verified: 2026-08-26` after confirming the prompt's description of the single-shot, no-tool, stdin/stdout contract still matches both scripts; no rule (7th occurrence of `stale-doc-prose`, closest-fit category for a doc-gardener-caught convention gap rather than wrong prose -- same reasoning as above, doc-gardener's missing-`Verified:` check is already the standing recurring guard) |
| 2026-08-26 | `stale-doc-prose` | `docs/references/builder-prompt.md` | doc-gardener | `Verified: 2026-08-24` fell behind because a referenced path, `docs/references/routine-prompt.md`, was touched the same day by an unrelated earlier fix (the previous log row above) -- content re-checked against both referenced files (`routine-prompt.md`, `scripts/check_builder_scope.py`) and found still accurate, no genuine drift; row preserved from PR #100 (Maintenance's own fix, correctly blocked by the critical-path gate and never merged) after the same `Verified:` bump landed independently via a separate edit | doc — bumped `Verified:` to 2026-08-26 after confirming accuracy; no rule (8th occurrence of `stale-doc-prose`, same reasoning as above -- doc-gardener's Verified-vs-source-commit-date check is already the standing recurring guard, prose correctness itself isn't mechanically checkable) |
| 2026-08-26 | `stale-doc-prose` | `docs/product-specs/reminders.md` | doc-gardener | flagged stale (source `docs/exec-plans/active/milestones.md` changed after `Verified:`); while re-checking, found the Non-goals section's "see ... M15" reference points at a milestone section that has never existed in `milestones.md` -- present since the spec was first added (#4, before M14 existed at all) and never caught because a milestone number isn't a path doc-gardener's link check can validate; the actual milestone covering reminders' Builder-built origin is M14 (added the same day, #108) | doc — corrected the reference to M14, bumped `Verified:` to 2026-08-26; no rule (9th occurrence of `stale-doc-prose`, same reasoning as above -- doc-gardener's Verified-vs-source-commit-date check is already the standing recurring guard for the staleness signal that surfaces this category, prose correctness itself isn't mechanically checkable, and a milestone-number cross-reference checker would need to parse arbitrary prose for "M<N>" mentions with no reliable delimiter -- too failure-prone to be clean and targeted) |
